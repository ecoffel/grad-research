...
for acInd = 1:length(restrictionData)
    ...
    for airportInd = 2:length(restrictionData{acInd}{2})
        ...
        % loop over months 
        for month = 1:size(restrictionData{acInd}{2}{airportInd}{2}{1}, 3)
            
            % calculate stats for the past (rcp = historical)
            % percentage of flights restricted
            prcRestPast(:, :, month, airportCount) = squeeze(nanmean(restrictionData{acInd}{2}{airportInd}{2}{1}(:, :, month, :), 2)) ./ ...
                                       squeeze(nanmean(restrictionData{acInd}{2}{airportInd}{2}{3}(:, :, month, :), 2)) .* 100;

            % percentage of TOW restricted
            meanTowRestPast(:, :, month, airportCount) = squeeze(nanmean(restrictionData{acInd}{2}{airportInd}{2}{2}(:, :, month, :), 2)) ./ ...
                                squeeze(nanmean(restrictionData{acInd}{2}{airportInd}{2}{4}(:, :, month, :), 2)) .* 100;

            prcPayloadFuelRestPast(:, :, month, airportCount) = squeeze(nanmean(restrictionData{acInd}{2}{airportInd}{2}{2}(:, :, month, :), 2)) .* 0.83 ./ divCoef ./ ...
                                payloadAndFuel .* 100;


            % stats for the future (rcp = rcpInd)
            % percentage of flights restricted
            prcRestFuture(:, :, month, airportCount) = squeeze(nanmean(restrictionData{acInd}{rcpInd}{airportInd}{2}{1}(:, :, month, :), 2)) ./ ...
                                       squeeze(nanmean(restrictionData{acInd}{rcpInd}{airportInd}{2}{3}(:, :, month, :), 2)) .* 100;

            % percentage of TOW restricted
            meanTowRestFuture(:, :, month, airportCount) = squeeze(nanmean(restrictionData{acInd}{rcpInd}{airportInd}{2}{2}(:, :, month, :), 2)) ./ ...
                                squeeze(nanmean(restrictionData{acInd}{rcpInd}{airportInd}{2}{4}(:, :, month, :), 2)) .* 100;

            prcPayloadFuelRestFuture(:, :, month, airportCount) = squeeze(nanmean(restrictionData{acInd}{rcpInd}{airportInd}{2}{2}(:, :, month, :), 2)) .* 0.83 ./ divCoef ./ ...
                                payloadAndFuel .* 100;
        end

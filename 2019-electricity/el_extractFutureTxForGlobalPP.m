...
for model = 1:length(models)
    ...
    for d = 1:size(decades, 1)
        ...
        if exist(['e:/data/ecoffel/data/projects/electricity/future-temps/' plantData '-pp-' rcp '-tx-cmip5-' models{model} '-' num2str(startYear) '-' num2str(endYear) '.csv'], 'file')
            %continue;
        end
        ...
        csvwrite(['e:/data/ecoffel/data/projects/electricity/future-temps/' plantData '-pp-' rcp '-tx-cmip5-' models{model} '-' num2str(startYear) '-' num2str(endYear) '.csv'], modelPlantTxTimeSeries);   
    end

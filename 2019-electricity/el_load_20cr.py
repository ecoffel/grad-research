...
tmax20CRDs = tmax20CRDs.where(tmax20CRDs['time.year'] <= 1900, drop=True)

print('loading selected data')
tmax20CRDs.load()

tmax20CR = np.full([ppLatLon.shape[0]+1, 51*12], np.nan)
...
for i, row in enumerate(ppLatLon):
    ...
    if i == 0:
        tmax20CR[0,:] = tmax20CRDs['time.month'].values
    tmax20CR[i+1, :] = curTmax

np.savetxt('%s/script-data/entsoe-nuke-pp-tx-20cr.csv'%dataDirDiscovery, tmax20CR, delimiter=',')

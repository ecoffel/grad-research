...
for i = 1:length(regionNames)
    ...
    if showPercentChange
        export_fig(['nile-seasonal-analysis-' regionAb{i} '-' tasmaxMetric '-' tasminMetric '-' anomalyStr '-percent.png'], '-m1');
    else
        export_fig(['nile-seasonal-analysis-' regionAb{i} '-' tasmaxMetric '-' tasminMetric '-' anomalyStr '-absolute.png'], '-m1');
    end
    close all;
end

...
for model = 1:length(models)
    ...
    if change
        ...
        if monthlyMean
            load([baseDir '/monthly-bowen-temp/monthlyBowenTemp-' rcpFuture '-' models{model} '-' timePeriodFuture '.mat']);
...
if plotEachModel
    ...
    export_fig(['r2-' regionAb{regionInd} '-' dataset '-' rcpStr '-BT-' monthlyMeanStr '-lag-' num2str(lag) '-NEW'  '.png'], '-m2');
end

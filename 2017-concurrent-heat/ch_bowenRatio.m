...
folDataTarget = [dataDir, '/bowen/regrid/world/', num2str(maxStartYear) num2str(maxStartMonth) '01-' num2str(minEndYear) num2str(minEndMonth) '31'];
...
while hfssStartInd <= hfssEndInd & hflsStartInd <= hflsEndInd
    ...
    eval([fileName ' = {hfssLat, hfssLon, bowen};']);
    save([bowenCurDir, '/', fileName, '.mat'], fileName, '-v7.3');

    eval(['clear ', fileName], ';');
    clear hfssLat hfssLon hfssData bowen;
    clear hflsLat hflsLon hflsData;

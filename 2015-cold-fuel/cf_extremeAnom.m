...
models = {'gfdl-cm3', 'cmcc-cm', 'cnrm-cm5', 'noresm1-m', 'mri-cgcm3', 'miroc-esm', 'mpi-esm-mr'};
...
basePeriod = 2006:2015;
futurePeriod = 2061:2069;
...
for m = 1:length(models)
    ...
    for y = basePeriod(1):yearStep:basePeriod(end)
        ['year ' num2str(y) '...']
        if baseRegrid
            baseDaily = loadDailyData([baseDir modelDir '/' curModel '/' ensemble '/' baseRcp '/' baseVar '/regrid'], 'yearStart', y, 'yearEnd', (y+yearStep)-1, 'plev', baseLev);
        else
            baseDaily = loadDailyData([baseDir modelDir '/' curModel '/' ensemble '/' baseRcp '/' baseVar], 'yearStart', y, 'yearEnd', (y+yearStep)-1, 'plev', baseLev);
        end
        if annualmean
    ...
    for y = testPeriod(1):yearStep:testPeriod(end)
        ...
        % load daily data
        if modelRegrid
            testDaily = loadDailyData([baseDir modelDir '/' curModel '/' ensemble '/' modelRcp '/' modelVar '/regrid'], 'yearStart', y, 'yearEnd', (y+yearStep)-1, 'plev', modelLev);
        else
            testDaily = loadDailyData([baseDir modelDir '/' curModel '/' ensemble '/' modelRcp '/' modelVar], 'yearStart', y, 'yearEnd', (y+yearStep)-1, 'plev', modelLev);
        end

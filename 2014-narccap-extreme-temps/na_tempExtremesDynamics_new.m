...
for d = 1:length(dataset)
    ...
    if statTest        
        ...
        % compare comp. anonaly to the std. matrix
        outputStatData{d} = [];
        for xlat = 1:size(outputCompGrid{d}, 1)
            for ylon = 1:size(outputCompGrid{d}, 2)
                if abs(outputCompGrid{d}(xlat, ylon)) > statTestThresh*compStdDev{d}{1}(xlat, ylon) & compStdDev{d}{1}(xlat, ylon) ~= 0
    ...
    % if we should conver the compGrid to show multiples of the std.
    % instead of the absolute anomalies
    if gridbox & ~realAnom
        % recalculate the output compGrid
        if diff
            
            outputCompGrid{d} = (compGrid{2} ./ compStdDev{d}{2}) - (compGrid{1} ./ compStdDev{d}{1});
        else
            outputCompGrid{d} = compGrid{1} ./ compStdDev{d}{1};
        end
    end
...
plotTitle='NARCCAP JJA soil moisture anomaly change';

result = {lat, lon, double(finalOutputCompData)};
saveData = {result, plotRegion, plotRange, plotTitle, fileTitle, plotXUnits, boxCoords, plotColorMap, blockWater, finalOutputStatData};

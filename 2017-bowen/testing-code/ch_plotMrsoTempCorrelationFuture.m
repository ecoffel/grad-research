...
if ~exist('tasmaxCmip5Historical', 'var')
    ...
    for m = 1:length(models)
        ...
        mrsoCmip5Historical(:, :, m, :) = squeeze(nanmean(curMrso{3}, 3));
        
        curMrso = loadMonthlyData(['e:/data/cmip5/output/' models{m} '/mon/r1i1p1/rcp85/mrso/regrid/world'], 'mrso', 'yearStart', 2060, 'yearEnd', 2079);
        % average over the years
        mrsoCmip5Future(:, :, m, :) = squeeze(nanmean(curMrso{3}, 3));
        
        tasmaxChg(:, :, m, :) = tasmaxCmip5Future(:, :, m, :) - tasmaxCmip5Historical(:, :,  m, :);
        mrsoChg(:, :, m, :) = (mrsoCmip5Future(:, :, m, :) - mrsoCmip5Historical(:, :, m, :)) ./ mrsoCmip5Historical(:, :, m, :) .* 100;
    end

    % loop over regions and extract bowen & tasmax change data
    for i = 1:length(regionNames)
        mrsoChgRegionsCmip5{i} = squeeze(mrsoChg(regionLatLonInd{i}{1}, regionLatLonInd{i}{2}, :, :)); 
...
for iind = 1:length(regionsToShow)
    ...
    mrsoCmip5Future = mrsoChgRegionsCmip5{i};
    ...
    % temp/mrso change for each grid box in each region and for each
    % season
    regionT = {[], [], [], []};
    regionM = {[], [], [], []};
    ...
    for season = 1:size(seasons, 1)
        ...
        for xlat = 1:size(tasmaxCmip5Future, 1)
            for ylon = 1:size(tasmaxCmip5Future, 2)
                ...
                regionM{season}(row, :) = squeeze(nanmean(mrsoCmip5Future(xlat, ylon, :, seasons(season, :)), 4));
    ...
    for season = 1:size(seasons, 1)
        for m = 1:length(models)
            ...
            curM = regionM{season}(:, m);
            nn = find(~isnan(curT) & ~isnan(curM));
            curT = curT(nn);
            curM = curM(nn);
            cors(m, season) = corr(curM, curT);
        end
    end
...
export_fig(['temp-mrso-corr-future.eps']);
close all;

...
for m = 1:length(models)
    ...
    if exist(['e:/data/projects/bowen/derived-chg/var-stats/qGroup-' models{m} '.mat'])
        %continue;
    end
    ...
    fprintf('loading %s...\n', models{m});
    qHist = loadDailyData(['e:/data/cmip5/output/' models{m} '/r1i1p1/historical/' var '/regrid/world'], 'startYear', 1981, 'endYear', 2005);
    qHist = qHist{3};
    ...
    for xlat = 1:size(lat, 1)
        for ylon = 1:size(lat, 2)            
            ...
            warmSeasonMean(xlat, ylon) = squeeze(nanmean(nanmean(nanmean(qHist(xlat, ylon, :, curTxxMonthsHist, :), 5), 4), 3));            
        end
    end
    ...
    for xlat = 1:size(lat, 1)
        for ylon = 1:size(lat, 2)            
            if isnan(warmSeasonMean(xlat, ylon))
                qGroup(xlat, ylon) = NaN;
            else
                qGroup(xlat, ylon) = find(abs(warmSeasonMean(xlat, ylon) - warmSeasonMeanP) == min(abs(warmSeasonMean(xlat, ylon) - warmSeasonMeanP)));
            end
        end
    end
    
    clear qHist;
    save(['e:/data/projects/bowen/derived-chg/var-stats/qGroup-' models{m} '.mat'], ['qGroup']);
end

...
if ~exist('tasmaxNcep', 'var')
    ...
    for i = 1:length(regionNames)
        ...
        tempRegionsNcep{i} = squeeze(tempNcep(regionLatLonInd{i}{1}, regionLatLonInd{i}{2}, :, :)); 
        mrsoRegionsNcep{i} = squeeze(mrsoNcep(regionLatLonInd{i}{1}, regionLatLonInd{i}{2}, :, :)); 

        tempRegionsEra{i} = squeeze(tempEra(regionLatLonInd{i}{1}, regionLatLonInd{i}{2}, :, :)); 
...
if ~exist('tasmaxCmip5', 'var')
    ...
    for i = 1:length(regionNames)
        tempRegionsCmip5{i} = squeeze(tempCmip5(regionLatLonInd{i}{1}, regionLatLonInd{i}{2}, :, :, :)); 
...
for i = [1, 2, 4, 7]
    ...
    tempNcep = tempRegionsNcep{i};
    mrsoEra = mrsoRegionsEra{i};
    tempEra = tempRegionsEra{i};
    mrsoCmip5 = mrsoRegionsCmip5{i};
    tempCmip5 = tempRegionsCmip5{i};
    ...
    for xlat = 1:size(mrsoNcep, 1)
        for ylon = 1:size(mrsoNcep, 2)
            ...
            for season = 1:size(seasons, 1)
                ...
                curB = squeeze(nanmean(tempNcep(xlat, ylon, :, seasons(season, :)), 4));
                ...
                curB = squeeze(nanmean(tempEra(xlat, ylon, :, seasons(season, :)), 4));
                ...
                for model = 1:length(models)
                    ...
                    curB = squeeze(nanmean(tempCmip5(xlat, ylon, model, :, seasons(season, :)), 5));

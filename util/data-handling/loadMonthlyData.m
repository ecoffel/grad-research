...
for d = 1:length(dirNames)
    ...
    elseif strfind(curDir, 'gpcp') ~= 0
        gpcp = true;
    elseif strfind(curDir, 'gpcc') ~= 0
        gpcc = true;
    elseif strfind(curDir, 'udel') ~= 0
        udel = true;
    ...
    for k = 1:length(matFileNames)
        ...
        elseif gpcp
            dataYear = str2num(matFileNameParts{2});
            dataMonth = str2num(matFileNameParts{3});
        elseif gpcc || udel || be || cpcMonthly || cru
            dataYear = str2num(matFileNameParts{2});
            dataMonth = str2num(matFileNameParts{3});
        elseif hadcrut
            dataYear = str2num(matFileNameParts{3});
...
%        ['loading ' matFileNameNoExt]
        ...
        if gpcp || gpcc || udel || be || hadcrut || cpcMonthly || cru

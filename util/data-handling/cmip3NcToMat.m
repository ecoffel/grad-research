function cmip3NcToMat(rawNcDir, outputDir, varName, maxNum)
...
for k = 1:length(ncFileNames)
    ...
    if length(parts) == 4
        ...
        emissionsScenario = lower(parts{2});
        timeSpacing = lower(parts{2});
        modelName = lower(parts{3});
        
        timeRange = strsplit(lower(parts{4}), '.');
    ...
    folDataTarget = [outputDir, '/', modelName, '/', emissionsScenario, '/', varName, '/', startDate, '-', endDate];

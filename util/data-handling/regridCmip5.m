            ...
            for r = 1:length(rcps)
                if length(plevs) > 0
                    for p = 1:length(plevs)
                        regridOutput(['e:/data/' modelBaseDir '/' models{m} '/' ensembles{e} '/' rcps{r} '/' regridVars{v}], regridVars{v}, baseGrid, 'skipexisting', true, 'plev', plevs{p}, 'latLonBounds', latLonBounds, 'v7', v7);
                    end
                else
                    regridOutput(['e:/data/' modelBaseDir '/' models{m} '/' ensembles{e} '/' rcps{r} '/' regridVars{v}], regridVars{v}, baseGrid, 'skipexisting', true, 'latLonBounds', latLonBounds, 'v7', v7);
                end
            end
        end
    end

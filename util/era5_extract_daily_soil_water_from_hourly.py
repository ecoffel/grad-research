...
for year in range(yearRange[0], yearRange[1]+1):
    ...
    if not os.path.isfile('%s/hourly/volumetric_soil_water_layer_1_hourly_%d.nc'%(dataDir, year)):
        print('skipping %s: file doesnt exist!'%('%s/hourly/volumetric_soil_water_layer_1_hourly_%d.nc'%(dataDir, year)))
        continue
    
    print('opening dataset for %d'%year)
    sm = xr.open_dataset('%s/hourly/volumetric_soil_water_layer_1_hourly_%d.nc'%(dataDir, year), mask_and_scale=True)

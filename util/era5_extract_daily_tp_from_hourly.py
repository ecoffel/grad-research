...
# IT SEEMS THIS DATA IS NOW IN UNITS OF m, NO NEED FOR SCALING (plus that needs 100GB+ of ram)

yearRange = [int(sys.argv[1]), int(sys.argv[2])]

for year in range(yearRange[0], yearRange[1]+1):
    
    if not os.path.isfile('%s/hourly/tp_hourly_%d.nc'%(dataDir, year)):
    ...
    tp = xr.open_dataset('%s/hourly/tp_hourly_%d.nc'%(dataDir, year))
...
#     tp = tp.astype(float) * scale + offset
    ...
    tp.to_netcdf('%s/daily/tp_%d.nc'%(dataDir, year), mode='w', encoding={'tp': {"dtype": "float32", "zlib": True, 'complevel':9}})

function dewPoint(dataDir, isRegridded, region)
...
tempDirNames = dir([dataDir '/' tempVar '/' regridStr region tempBc '/']);
...
for d = 1:length(tempDirNames)
    ...
    tempCurDir = [dataDir '/' tempVar '/' regridStr region tempBc '/' tempDirNames{d}];
...
folDataTarget = [dataDir, '/dwpt/', regridStr, region, '/', num2str(maxStartYear) num2str(maxStartMonth) '01-' num2str(minEndYear) num2str(minEndMonth) '31'];
...
while tempStartInd <= tempEndInd & rhStartInd <= rhEndInd
    ...
    fileName = ['dwpt_', num2str(curYear), '_' monthStr, '_01'];
    
    if exist([dwptCurDir '/' fileName '.mat'], 'file')
        ['skipping ' dwptCurDir '/' fileName '.mat']
        tempStartInd = tempStartInd + 1;
        rhStartInd = rhStartInd + 1;
        continue;
    end
    ...
    ['processing ' dwptCurDir '/' fileName]
    eval([fileName ' = {tempLat, tempLon, dwpt};']);
    save([dwptCurDir, '/', fileName, '.mat'], fileName, '-v7.3');

    eval(['clear ', fileName], ';');
    clear tempLat tempLon tempData dwpt;
    clear rhLat rhLon rhData;
